...
def showConfusionMatrix(p_a_ls):
    ...
    for i in range(10):
        TP = p_a_ls.filter(p_a_ls['label'] == i and p_a_ls['prediction'] == i).count()
        TN = p_a_ls.filter(p_a_ls['label'] != i and p_a_ls['prediction'] != i).count()
        FP = p_a_ls.filter(p_a_ls['label'] != i and p_a_ls['prediction'] == i).count()
        FN = p_a_ls.filter(p_a_ls['label'] == i and p_a_ls['prediction'] != i).count()
        precision = TP / (TP + FP)
        recall = TP / (TP + FN)
